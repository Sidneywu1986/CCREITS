#!/usr/bin/env python3
"""
同步 we-mp-rss 文章到主库 —— SQLite → SQLite 版
...
# 目标数据库（主库 SQLite）
TARGET_DB_PATH = os.getenv("DATABASE_URL", "sqlite:///./database/reits.db")
if TARGET_DB_PATH.startswith("sqlite:///"):
    TARGET_DB_PATH = TARGET_DB_PATH.replace("sqlite:///", "")
    if not os.path.isabs(TARGET_DB_PATH):
        TARGET_DB_PATH = os.path.join(os.path.dirname(__file__), "..", TARGET_DB_PATH)
...
def ensure_target_table(conn):
    ...
    CREATE INDEX IF NOT EXISTS idx_wechat_vectorized 
        ON wechat_articles(vectorized) WHERE vectorized = 0;
    CREATE INDEX IF NOT EXISTS idx_wechat_published 
        ON wechat_articles(published DESC);
    CREATE INDEX IF NOT EXISTS idx_wechat_source 
        ON wechat_articles(source);
    """
...
def upsert_to_target(target_conn, articles: List[Dict]):
    rows = []
    for a in articles:
        ...
        rows.append((
            a["source"], a["title"], a["link"], published,
            content, a["raw_content"], length
        ))

    sql = """
    INSERT OR IGNORE INTO wechat_articles
        (source, title, link, published, content, raw_content, content_length)
    VALUES (?, ?, ?, ?, ?, ?, ?)
    """
    with target_conn:
        target_conn.executemany(sql, rows)


def get_last_sync_time(target_conn) -> Optional[datetime]:
    cur = target_conn.cursor()
    cur.execute("SELECT MAX(published) FROM wechat_articles")
    row = cur.fetchone()
    if row and row[0]:
...
def main():
    ...
    logger.info(f"目标库: {TARGET_DB_PATH}")
    ...
    try:
        ...
        last_sync = get_last_sync_time(target_conn)
        ...
        if articles:
            upsert_to_target(target_conn, articles)
            # Count how many were actually inserted
            cur = target_conn.cursor()
            cur.execute("SELECT COUNT(*) FROM wechat_articles")
